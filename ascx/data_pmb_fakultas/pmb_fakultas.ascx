...
Private Const FAK_ALL As String = "(Semua Fakultas)"
Private SelectedFak As String = FAK_ALL
...
Private Sub PopulateFakultasFilter()
    Dim listF As New List(Of String)()
    Using cs As New OleDb.OleDbConnection(ConnStar)
        cs.Open()
        Using cmd As New OleDb.OleDbCommand(
            "SELECT DISTINCT ISNULL(nm_fak,'Tidak Terpetakan') AS fak " &
            "FROM dim_prodi ORDER BY fak", cs)
            Using rd = cmd.ExecuteReader()
                While rd.Read()
                    listF.Add(rd("fak").ToString())
                End While
            End Using
        End Using
    End Using
    ...
    ddlFak.Items.Clear()
    ddlFak.Items.Add(New ListItem(FAK_ALL, FAK_ALL))
    For Each f In listF
        ddlFak.Items.Add(New ListItem(PrettyFak(f), f))
    Next

    Dim def As String = If(ViewState("FAK") Is Nothing, FAK_ALL, ViewState("FAK").ToString())
    If ddlFak.Items.FindByValue(def) Is Nothing Then def = FAK_ALL
    ddlFak.SelectedValue = def
    SelectedFak = def
End Sub
...
Sub Page_Load(sender As Object, e As EventArgs)
    ...
    Else
        ...
        If ViewState("FAK") IsNot Nothing Then SelectedFak = ViewState("FAK").ToString()
        EnsureRangeOrder()
    End If
End Sub
...
Protected Sub OnFakChanged(sender As Object, e As EventArgs)
    SelectedFak = ddlFak.SelectedValue
    ViewState("FAK") = SelectedFak
    LoadHeaderTiles()
    BuildGauges()
    BuildCharts()
End Sub

Private Function AppendFakFilter(sql As String, aliasDp As String) As String
    If String.IsNullOrEmpty(SelectedFak) OrElse SelectedFak = FAK_ALL Then Return sql
    Return sql & " AND ISNULL(" & aliasDp & ".nm_fak,'Tidak Terpetakan') = ? "
End Function

Private Sub AddFakParamIfNeeded(cmd As OleDb.OleDbCommand)
    If String.IsNullOrEmpty(SelectedFak) OrElse SelectedFak = FAK_ALL Then Exit Sub
    cmd.Parameters.AddWithValue("@pfak", SelectedFak)
End Sub
...
Private Sub BuildCharts()
    ...
    Using cs As New OleDb.OleDbConnection(ConnStar)
        ...
        Dim sql = "SELECT w.tahun, COUNT(*) AS jml " &
                  ...
                  "LEFT JOIN dim_prodi dp ON dp.kd_jur=f.kd_jur " &
                  "WHERE w.tahun BETWEEN ? AND ? AND f.kd_jur IS NOT NULL "
        sql = AppendFakFilter(sql, "dp") & "GROUP BY w.tahun ORDER BY w.tahun"
    ...
    Using cp As New OleDb.OleDbConnection(ConnPmb)
        ...
        Dim sql = "SELECT k.tahun, ISNULL(SUM(k.target_total),0) AS tgt " &
                  ...
                  "  OR LTRIM(RTRIM(k.kd_jur)) = LTRIM(RTRIM(dp.kode_jurnim)) " &
                  "WHERE k.tahun BETWEEN ? AND ? "
        sql = AppendFakFilter(sql, "dp") & "GROUP BY k.tahun ORDER BY k.tahun"
...
Using cs As New OleDb.OleDbConnection(ConnStar)
  ...
  Dim sqlS As String = _
    ...
    "JOIN dim_prodi  dp ON dp.kd_jur   = f.kd_jur " & _
    "JOIN dim_sekolah s ON s.id_sekolah= f.id_sekolah " & _  
    "WHERE w.tahun BETWEEN ? AND ? " & _
    "  AND f.kd_jur IS NOT NULL " & _
    "  AND LTRIM(RTRIM(ISNULL(s.nm_sekolah,''))) <> '' " & _
    "  AND UPPER(REPLACE(REPLACE(LTRIM(RTRIM(s.nm_sekolah)),'(',''),')','')) NOT IN " & _
    "      ('TIDAK DIKETAHUI','TIDAK TERPETAKAN','LAIN-LAIN','LAINNYA','-') "

  sqlS = AppendFakFilter(sqlS, "dp") & _
...
Using cs As New OleDb.OleDbConnection(ConnStar)
  ...
  ' Bangun SELECT dalam (subquery) supaya filter fakultas bisa disisipkan
  Dim innerP As String = _
    "SELECT " & _
    "  CASE " & _
    "    WHEN p.id_level_wil IN (2,3) THEN LTRIM(RTRIM(p.nm_wil)) " & _  
    "    WHEN p.id_level_wil = 4 THEN LTRIM(RTRIM(p2.nm_wil)) " & _      
    "    WHEN p.id_level_wil = 5 THEN LTRIM(RTRIM(p3.nm_wil)) " & _      
    "    ELSE LTRIM(RTRIM(p.nm_wil)) " & _
    "  END AS prov " & _
    "FROM fact_pmb f " & _
    "JOIN dim_waktu  w  ON w.id_waktu = f.id_waktu " & _
    "LEFT JOIN dim_prodi  dp ON dp.kd_jur = f.kd_jur " & _
    "LEFT JOIN dim_wilayah p  ON p.id_wil = f.id_wil " & _
    "LEFT JOIN dim_wilayah p2 ON p2.id_wil = p.id_induk_wilayah " & _
    "LEFT JOIN dim_wilayah p3 ON p3.id_wil = p2.id_induk_wilayah " & _
    "WHERE w.tahun BETWEEN ? AND ? " & _
    "  AND f.kd_jur IS NOT NULL "

  innerP = AppendFakFilter(innerP, "dp")  

  Dim sqlP As String = _
    "SELECT TOP 10 prov AS provinsi, COUNT(*) AS jml " & _
    "FROM (" & innerP & ") X " & _
    "WHERE LTRIM(RTRIM(ISNULL(prov,''))) <> '' " & _
    "  AND UPPER(prov) NOT IN ('TIDAK TERPETAKAN','TIDAK DIKETAHUI','LAIN-LAIN','LAINNYA','-') " & _
    "GROUP BY prov " & _
  ...
  Using cmd As New OleDb.OleDbCommand(sqlP, cs)
    cmd.Parameters.AddWithValue("@p1", TahunFrom)
    cmd.Parameters.AddWithValue("@p2", TahunTo)
    AddFakParamIfNeeded(cmd) ' jika user memilih 1 fakultas
    Using rd = cmd.ExecuteReader()
      While rd.Read()
...
<section class="content-header">
  <h1>DASHBOARD MAHASISWA BARU FAKULTAS</h1>
    <ol class="breadcrumb">
        <li><a href="/dashboard_pmb/index.aspx"><i class="fa fa-dashboard"></i> Dashboard PMB</a></li>
        <li class="active">Dashboard Mahasiswa Baru Fakultas</li>
    </ol>
</section>
  ...
  <div class="col-md-12">
    <div class="filter-range form-inline">
      ...
      <asp:DropDownList ID="ddlYearTo" runat="server"
        ...
        AutoPostBack="true" OnSelectedIndexChanged="OnYearChanged" />
      <label style="margin:8px 8px 0 16px">Fakultas</label>
      <asp:DropDownList ID="ddlFak" runat="server"
        CssClass="form-control input-sm"
        AutoPostBack="true" OnSelectedIndexChanged="OnFakChanged" />
    </div>
  </div>
</div>
        ...
        <div class="box-body">
            <div style="height:120px"><canvas id="gaugeProgress"></canvas></div>
                <small class="text-muted">
                  Realisasi:
                  <%= TotalRealisasi.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("id-ID")) %>
                  (<%= PctRealisasiText %>) |
                  Target:
                  <%= TotalTarget.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("id-ID")) %>
                  (100%) |
                  Sisa:
                  <%= SisaAbs.ToString("N0", System.Globalization.CultureInfo.GetCultureInfo("id-ID")) %>
                  (<%= PctSisaText %>)
              </small>
            </div>
        </div>
    ...
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <% If SelectedFak <> FAK_ALL Then %>
                    &ndash; <%= PrettyFak(SelectedFak).Replace("Fakultas ","") %>
                <% End If %>
                </h3>
            </div>
...
<script>
  ...
  const CenterText = {
    ...
    afterDatasetsDraw(chart, args, opts){
      ...
      const y = (top + bottom) / 2;  // cukup pas utk semi-doughnut
      ctx.save();
      ctx.font = (opts.font || '600 20px system-ui, Arial');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // === chart lain tetap seperti sebelumnya ===
    var cfgs = <%= chartConfigJson %>;
    cfgs.forEach(function(c){
      var dl = { anchor:'end', align:'end', clamp:true, clip:false, font:{weight:'600'},
      ...
      if (el2) new Chart(el2, { type:c.type, data:c.data, options:c.options });
    });
  });
</script>
